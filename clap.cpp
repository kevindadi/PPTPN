...
void TDG::parse_tdg_from_string(std::string dot_content) {
    ...
    // 注意区分文件流和字符串流!!!
    std::istringstream tdg_stream(dot_content);
    ...
    if (read_graphviz(tdg_stream, tdg, tdg_dp)) {
        qDebug() << "Graph Name: " << get_property(tdg, boost::graph_name);
        // 遍历节点，确定节点类型
        BOOST_FOREACH (TDG_RAP::vertex_descriptor v, vertices(tdg)) {
            ...
            try {
                ...
                throw LabelParseException("Label parsing error for node " + id + ": " + ex.label);
            } catch (const TimeValueException &ex) {
                throw TimeValueException("Time value error in label for node " + id + ": " + ex.time_values, ex.time_values);
            } catch (const boost::exception &ex) {
                throw LabelParseException("Boost Exception caught: " + boost::diagnostic_information(ex));
            } catch (const std::exception &ex) {
                throw LabelParseException("Standard Exception caught: " + std::string(ex.what()));
            }
...
NodeType TDG::parse_vertex_label(const string &label) {
    if (label.empty()) {
        throw LabelParseException("Label cannot be empty");
    }
    ...
    if (regex_search(label, matches, rgx)) {
        ...
        if (content.empty()) {
            throw LabelParseException("Empty content in label: " + label);
        }
        ...
        // 检查是否至少包含名称
        if (parts.empty()) {
            throw LabelParseException("No task name found in label");
        }

        string name = parts[0];
        if (name.empty()) {
            throw LabelParseException("Task name cannot be empty");
        }
        ...
        if (has_period) {
            ...
            // 处理非周期性任务
            if (parts.size() < 4) {
                throw LabelParseException("Insufficient parameters for aperiodic task: " + name);
            }
    ...
    throw LabelParseException("Invalid label format: " + label);
}

vector<int> TDG::parse_time_vec(string times_string) {
    if (times_string.empty()) {
        throw TimeValueException("Empty time string", "Time string cannot be empty");
    }
    ...
    try {
        ...
        while (std::regex_search(start, end, matches, rgx)) {
            ...
            if (matches.size() != 3) { // 完整匹配 + 两个捕获组
                throw TimeValueException("Invalid time format", "Expected format: [number,number]");
            }
            ...
            try {
                ...
                throw TimeValueException("Invalid number format",
                                       "Failed to convert string to integer in: " + times_string);
            } catch (const std::out_of_range&) {
                throw TimeValueException("Number out of range",
                                       "Number too large in: " + times_string);
            }

            // 验证时间值的合理性
            if (first_value < 0 || second_value < 0) {
                throw TimeValueException("Invalid time value", "Time values must be non-negative");
            }

            if (first_value > second_value) {
                throw TimeValueException("Invalid time range",
        ...
        if (!found_match) {
            throw TimeValueException("No valid time ranges found",
                                   "Input string does not contain any valid time ranges: " + times_string);
        }

        // 检查结果向量的合理性
        if (values.size() % 2 != 0) {
            throw TimeValueException("Invalid number of values",
                                   "Number of parsed values must be even");
        }

    } catch (const std::regex_error& e) {
        throw TimeValueException("Regex error",
