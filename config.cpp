...
void Config::parse_dag() {
  boost::ref_property_map<DAG *, std::string> dag_name(get_property(dag, boost::graph_name));
  ...
  if (read_graphviz(dag_f, dag, dag_dp)) {
    BOOST_LOG_TRIVIAL(info) << "Graph Name: " << get_property(dag, boost::graph_name);
...
void Config::parse_json() {
  ...
  for (const auto& entry : jsonData) {
    ...
    for (const auto& time : time1Strings) {
      ...
      if (units.size() == 2) {
        data.time.emplace_back(stoi(units[0]),
                                 stoi(units[1]));
      }
    }
...
//      }
    ...
    if (data.lock.size() + 1 != data.time.size()) {
      ...
      BOOST_LOG_TRIVIAL(error) << "Error: time1 size should be one more than time2 size";
...
std::map<int, std::vector<TaskConfig>> Config::classify_priority() {
  ...
  for (auto &it : tp) {
    std::sort(it.second.begin(), it.second.end(), [](const TaskConfig &task1, const TaskConfig &task2) {
