...
QStringList StateClassGraph::generate_state_class() {
    QStringList sl;
    ...
    boost::dynamic_properties scg_dp;
    scg_dp.property("node_id", get(&SCGVertex::id, scg));
    scg_dp.property("label", get(&SCGVertex::label, scg));

    boost::ref_property_map<SCG *, std::string> gname_pn(
        get_property(scg, boost::graph_name));
    scg_dp.property("name", gname_pn);

    boost::filesystem::path current_path = boost::filesystem::current_path();

    // 生成文件名（可以使用日期、时间戳等来生成唯一的文件名）
    std::string filename = "scg_graph.dot";
    boost::filesystem::path full_path = current_path / filename;

    std::ofstream ofs;
    ofs.open(full_path.string());
    if (!ofs) {
        BOOST_LOG_TRIVIAL(error) << "Failed to open file for writing!" << std::endl;
        return {};
    }

    boost::write_graphviz_dp(ofs, scg, scg_dp);

    // 输出统计信息
    auto duration = chrono::duration<double>(chrono::steady_clock::now() - start_time);
    sl.append("State class generation completed: ");
    sl.append("- Time taken: " + QString::number(duration.count()) + "s");
    sl.append("- Total states: " + QString::number(num_vertices(scg)));
    sl.append("- Unique states: " + QString::number(scg_vertex_map.size()));
    return sl;
}
