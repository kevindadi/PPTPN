...
find_task_vertex(SCG &Scg, std::string task_name) {
  // 构建要搜索的标记
...
void dfs_all_path(SCG &scg, ScgVertexD start, ScgVertexD end,
                  std::vector<Path> &all_paths, Path &current_path,
                  std::vector<bool> &visited, const std::string &exit_flag) {
  visited[start] = true;
  ...
  if (start == end || scg[start].id.find(exit_flag) != std::string::npos) {
    ...
    for (auto [ei, ei_end] = boost::out_edges(start, scg); ei != ei_end; ++ei) {
      ...
      if (!visited[next]) {
        ...
        dfs_all_path(scg, next, end, all_paths, current_path, visited,
                     exit_flag);
      }
    }
  }
...
std::pair<int, std::vector<Path>> calculate_wcet(SCG &scg, ScgVertexD start,
                                                 ScgVertexD end,
  ...
  for (const auto &path : all_paths) {
    // 计算当前路径的执行时间
    int path_weight = std::accumulate(path.begin(), path.end() - 1, 0,
...
int task_wcet(SCG &scg, std::string start_task, std::string exit_task) {
  ...
  for (const auto &[start, end] : vertex_pairs) {
    futures.push_back(pool.enqueue([&scg, start, end, exit_flag]() {
