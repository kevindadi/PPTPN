...
std::pair<DBM, DBM> StateClassReachabilityGraph::time_advance(const StateClass& state) const {
    ...
    for (size_t i = 1; i < num_clocks && i <= num_transitions; ++i) {
        ...
        if (trans_idx < num_transitions) {
            is_enabled = matrix_ptpn::MatrixPTPN::is_enabled(state.marking, ptpn_, trans_idx);
        }
        ...
        const auto& transition = ptpn_.get_transition(trans_idx);
        bool is_exact_time = (transition.time_interval.earliest == transition.time_interval.latest &&
    ...
    BOOST_LOG_TRIVIAL(debug) << "  时间推进: 放宽了 " << relaxed_count << " 个时钟的上界";
...
void StateClassReachabilityGraph::update_dbm_constraints(StateClass& state) {
    ...
    if (cleared_count > 0) {
        BOOST_LOG_TRIVIAL(debug) << "    清除了 " << cleared_count << " 个不再使能变迁的约束";
    }
...
std::vector<size_t> StateClassReachabilityGraph::select_per_core(const std::set<size_t>& enabled) const {
    // 每个核心内挑选最高优先级候选
...
void StateClassReachabilityGraph::apply_preemption(const std::vector<size_t>& chosen, StateClass& state) const {
...
void StateClassReachabilityGraph::compute_enabled_and_clocks(StateClass& state) {
    ...
    BOOST_LOG_TRIVIAL(debug) << "    计算使能和时钟: 使能=" << state.enabled.size() 
                              << "个变迁";
}
