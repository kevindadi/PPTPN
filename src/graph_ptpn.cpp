...
static VertexDesc add_place(Graph& graph, const std::string& name, int token = 0, int capacity = 1) {
    Vertex v;
    v.name = name;
    v.label = name;
    v.shape = "circle";
...
bool GraphPTPN::save_to_dot(const std::string& file_path) const {
    try {
        ...
        if (!ofs) {
            BOOST_LOG_TRIVIAL(error) << "[GRAPH_PTPN] 无法打开DOT文件: " << dot_filename.string();
            return false;
        }
