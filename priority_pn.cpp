...
//// 根据优先级,建立抢占变迁, 绑定锁
/// \param tdg
QStringList PriorityTimePetriNet::transform_tdg_to_ptpn(TDG &tdg) {
    try {
        // 1. 转换顶点
        ...
        // 5. 输出网络信息
        QStringList info = log_network_info();
        QString pn_file = save_ptpn_to_dot();
        info.append("文件保存到: ");
        info.append(pn_file);
        return info;
...
QStringList PriorityTimePetriNet::log_network_info() {
    QStringList s;
    s.append("Petri 网信息统计:");
    s.append("- 库所 + 变迁数量: " + QString::number(boost::num_vertices(ptpn)));
    s.append("- 流数量: " + QString::number(boost::num_edges(ptpn)));
    return s;
}
...
QString PriorityTimePetriNet::save_ptpn_to_dot() {
    ...
    std::string filename = "ptpn_graph.dot";
    ...
    boost::write_graphviz_dp(ofs, ptpn, ptpn_dp);

    return QString::fromStdString(full_path.string());
}
