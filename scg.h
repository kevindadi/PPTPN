...
public:
    ...
    StateClassGraph(StateClassGraph&& other) noexcept
        : init_ptpn(std::move(other.init_ptpn)),   // 转移 unique_ptr 所有权
        scg(std::move(other.scg)),              // 假设 SCG 支持移动语义
        init_state_class(std::move(other.init_state_class)) {  // 转移状态类
        // 确保被移动的源对象处于有效状态
        // 此处无需额外操作，unique_ptr 移动后源对象指针自动为 nullptr
    }
    // 状态类图,insert唯一的stateclass
    SCG scg;

    // 生成状态类图主函数
    QStringList generate_state_class();
