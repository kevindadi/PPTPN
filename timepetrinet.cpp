...
void TimePetriNet::init_graph() {
  ...
  boost::ref_property_map<TPN *, std::string> gname_pn(
      get_property(time_petri_net, boost::graph_name));
...
void TimePetriNet::construct_petri_net(const Config &config) {
  ...
  for (const auto &id : task_name) {
    if (id.second.find("Wait") != std::string::npos) {
      ...
      //      vertex_tpn wait_v_l = add_transition(time_petri_net, wait2, false,
      //      pt);
      // add_edge(wait_v_s, wait_v_m, time_petri_net);
      // add_edge(wait_v_m, wait_v_l, time_petri_net);
      id_start_end.insert(
          std::pair<std::size_t, boost::tuple<vertex_tpn, vertex_tpn>>(
              id.first, boost::tie(wait_v_s, wait_v_s)));

      id_str_start_end.insert(
          std::make_pair(id.second, std::make_pair(wait_v_s, wait_v_s)));
    ...
    if (id.second.find("Distribute") != std::string::npos) {
      ...
      id_start_end.insert(
          std::pair<std::size_t, boost::tuple<vertex_tpn, vertex_tpn>>(
              id.first, boost::tie(dist_v_s, dist_v_s)));

      id_str_start_end.insert(
          std::make_pair(id.second, std::make_pair(dist_v_s, dist_v_s)));
    ...
    vertex_tpn get_core = add_transition(
        time_petri_net, task_get, false,
        TPetriNetTransition{0, task_pror, std::make_pair(0, 0), task_core});
    vertex_tpn ready = add_place(time_petri_net, task_ready, 0);

    vertex_tpn exec = add_transition(
        time_petri_net, task_exec, false,
    ...
    if (tc_t.lock.empty()) {
      ...
      for (int j = (lock_n - 1); j >= 0; j--) {
        ...
        vertex_tpn drop_lock =
            add_transition(time_petri_net, task_drop + lock_name, false,
    ...
    id_start_end.insert(
        std::pair<std::size_t, boost::tuple<vertex_tpn, vertex_tpn>>(
            id.first, boost::tie(entry, exit)));
    id_str_start_end.insert(
        std::make_pair(id.second, std::make_pair(entry, exit)));
  ...
  for (boost::tie(ei, ei_end) = edges(config.dag); ei != ei_end; ++ei) {
    ...
    if (source_vertex == target_vertex) {
      ...
      vertex_tpn v2 = add_transition(
          time_petri_net, n_timer, false,
          TPetriNetTransition{0, 256, std::make_pair(spin_time, spin_time),
                              256});
      ...
      std::string source_vertex_name =
          (*(id_name_map_me.find(source_vertex))).second;
      ...
      if ((source_vertex_name.find("Wait") == 0) ||
        ...
        vertex_tpn link = add_transition(
            time_petri_net, link_name, false,
  ...
  BOOST_LOG_TRIVIAL(info) << "petri net P+T num: "
                          << boost::num_vertices(time_petri_net);
  BOOST_LOG_TRIVIAL(info) << "petri net F num: "
                          << boost::num_edges(time_petri_net);
  std::ofstream os;
  os.open("t.dot");
        ...
        for (auto it : it1_vertex->second) {
          ...
          if (tc_it1.lock.empty()) {
            ...
            std::string task_drop =
                it2->name + "drop_lock" + std::to_string(element_id);
            ...
            vertex_tpn get_core = add_transition(
                time_petri_net, task_get, false,
                TPetriNetTransition{0, task_pror, std::make_pair(0, 0),
                                    task_core});
            vertex_tpn ready = add_place(time_petri_net, task_ready, 0);
            ...
            if (tc_it2.lock.empty()) {
              ...
              for (int j = 0; j < tc_it2.lock.size(); j++) {
                ...
                vertex_tpn get_lock = add_transition(
                    time_petri_net,
                    task_lock + lock_name + std::to_string(element_id), false,
                    TPetriNetTransition{0, 256, {0, 0}, task_core});
                vertex_tpn deal = add_place(
                    time_petri_net,
                    task_deal + lock_name + std::to_string(element_id), 0);
                node.push_back(get_lock);
                node.push_back(deal);
                add_edge(node.back(), get_lock, time_petri_net);
                add_edge(get_lock, deal, time_petri_net);
              ...
              for (int k = (tc_it2.lock.size() - 1); k >= 0; k--) {
                ...
                vertex_tpn drop_lock = add_transition(
                    time_petri_net,
                    task_drop + lock_name + std::to_string(element_id), false,
                    TPetriNetTransition{0, 256, l_t, task_core});
                vertex_tpn unlocked = add_place(
                    time_petri_net,
            ...
            if (record <= 0) {
              for (int p = 2, q = (task_v_num - 3), l = 0; l <= lock_n;
                ...
                create_priority_task(config, it2->name, preempt_vertex_front, p,
                                     (it2_vertex->second)[0][0],
                                     (it2_vertex->second)[0].back());
                create_priority_task(config, it2->name, preempt_vertex_back, q,
                                     (it2_vertex->second)[0][0],
                                     (it2_vertex->second)[0].back());
              }
            } else {
...
void TimePetriNet::create_priority_task(const Config &config,
  ...
  vertex_tpn exec =
      add_transition(time_petri_net, task_exec, false,
...
std::vector<SchedT> TimePetriNet::get_sched_t(StateClass &state) {
  ...
  //      if ((time_petri_net[sched_T[i].t].pnt.priority <
  //      time_petri_net[sched_T[j].t].pnt.priority) &&
  //            (time_petri_net[sched_T[i].t].pnt.c ==
  //            time_petri_net[sched_T[j].t].pnt.c)) {
...
StateClass TimePetriNet::fire_transition(const StateClass &sc,
  ...
  //        if ((time_petri_net[enabled_t[i]].pnt.priority <
  //        time_petri_net[enabled_t[j]].pnt.priority) &&
  //            (time_petri_net[enabled_t[i]].pnt.c ==
  //            time_petri_net[enabled_t[j]].pnt.c)) {
  //        erase_index.push_back(i);
  //        }
  //    }
  //  }
  ...
  //      if ((time_petri_net[(*it).t].pnt.priority <
  //      time_petri_net[(*it2).t].pnt.priority) &&
