...
struct Transition {
    ...
    Transition(const std::string& id = "", 
        ...
        : id(id), name(name), time_interval(interval), 
...
public:
    ...
    size_t add_transition(const std::string& name,
                          ...
                          int core = 0,
                          bool suspendable = false) {
        transitions.emplace_back(std::to_string(transitions.size()), name, 
                                 interval, priority, core, suspendable);
    ...
    [[nodiscard]] const std::vector<std::vector<int>>& get_pre_matrix() const { return Pre; }
    [[nodiscard]] const std::vector<std::vector<int>>& get_post_matrix() const { return Post; }

    static bool is_enabled(const Marking& M, const MatrixPTPN& net, size_t trans_idx) {
    ...
    static Marking fire(const Marking& M, const MatrixPTPN& net, size_t trans_idx) {
        ...
        for (size_t p = 0; p < net.places.size(); ++p) {
            ...
            if (net.places[p].capacity != INF && new_marking[p] > net.places[p].capacity) {
    ...
    [[nodiscard]] std::vector<size_t> filter_by_priority(const std::vector<size_t>& enabled_transitions) const {
...
private:
    ...
    void add_monitor_matrix(const std::string& task_name, int task_period_time, size_t start, size_t end);
    void add_preempt_task_matrix(const std::unordered_map<int, std::vector<std::string>>& core_task,
    ...
    void task_bind_lock_resource_matrix(const std::vector<NodeType>& all_task,
                                       std::map<std::string, std::vector<std::string>>& task_locks);
    void bind_task_locks_matrix(const std::string& task_name,
    ...
    void handle_self_loop_edge_matrix(TDG& tdg, void* edge_desc, const std::string& source_name);  // edge_desc 在 cpp 中转换为 TDG_RAP::edge_descriptor
    void handle_dashed_edge_matrix(const std::string& source_name, const std::string& target_name);
...
private:
    ...
    std::unordered_map<std::string, std::vector<size_t>> node_pn_map;     // 节点名称 -> Petri网链
